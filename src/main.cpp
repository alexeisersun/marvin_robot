...
void ChassisStop(void) { // steps in ms
// stop motors
motorRight.stop();
motorLeft.stop();
}

void ChassisStepFWD(int steps) { // steps in ms
...
void ChassisStepBWD(int steps) { // steps in ms
...
void ChassisTurnLeft(int angle) { // angle in ms
...
} void ChassisTurnRight(int angle) { // angle in ms
...
if ( duration <= 0) duration = MAX_DUR;
...
case 'F': CTRL_SERIAL.println("Am receptiont Comanda FWD");
ChassisStepFWD(PROG_WALK_STEP);
break;

case 'B': CTRL_SERIAL.println("Am receptiont Comanda BWD");
ChassisStepBWD(PROG_WALK_STEP);
break;

case 'L': CTRL_SERIAL.println("Am receptiont Comanda LEFT");
ChassisTurnLeft(PROG_TURN_ANGLE);
break;

case 'R': CTRL_SERIAL.println("Am receptiont Comanda RIGHT");
ChassisTurnRight(PROG_TURN_ANGLE);
break;

case 'S': CTRL_SERIAL.println("Am receptiont Comanda STOP");
ChassisStop();
break;
}
progIt++;
...
// } else
if (minID == SENS_US_LEFT_ID) { // Angle Under
ChassisNavSpeedInit();
ChassisTurnLeft(NAV_TURN_ANGLE);
} else if (minID == SENS_US_RIGHT_ID) { // Angle Over
ChassisNavSpeedInit();
ChassisTurnRight(NAV_TURN_ANGLE);
} else if (minDist > DISTANCE_MAX) { // Distance Over
ChassisStepFWD(NAV_WALK_STEP);
ChassisNavSpeedUp();
} else if (minDist < DISTANCE_MIN) { // Distance under
ChassisNavSpeedInit();
...
// ChassisStepFWD(NAV_WALK_STEP);
// } else
if (maxID == SENS_US_LEFT_ID) { // Angle Under
ChassisNavSpeedInit();
ChassisTurnLeft(NAV_TURN_ANGLE);
} else if (maxID == SENS_US_RIGHT_ID) { // Angle Over
ChassisNavSpeedInit();
ChassisTurnRight(NAV_TURN_ANGLE);
} else if (maxDist > DISTANCE_MAX) { // Distance Over
ChassisStepFWD(NAV_WALK_STEP);
ChassisNavSpeedUp();
} else if (maxDist < DISTANCE_MIN) { // Distance under
ChassisStepBWD(NAV_WALK_STEP);
} else {
ChassisStop();
...
case '0': CTRL_SERIAL.println("Am receptiont Comanda 0");
PlayMorseString("SOS");
break;

case '1': CTRL_SERIAL.println("Am receptiont Comanda 1 - Manual Control");
APP_STATE = APP_STATE_MANUAL;
break;

case '2': CTRL_SERIAL.println("Am receptiont Comanda 2 - Follower");
APP_STATE = APP_STATE_AUTO_FOLLOW;
break;

case '3': CTRL_SERIAL.println("Am receptiont Comanda 3 - Tunel");
APP_STATE = APP_STATE_AUTO_TUNEL;
break;

case 'P': CTRL_SERIAL.println("Am receptiont Comanda P - PROGRAM");
ChassisNavProgram("FFFLRFFFLBBBBBB");
break;

case 'Y': CTRL_SERIAL.println("Am receptiont Comanda y - Play Log");
ChassisNavProgram(log_program);
break;

case 'X': CTRL_SERIAL.println("Am receptiont Comanda x - clean");
log_program[0]=0;
break;

case 'C': CTRL_SERIAL.println("Am receptiont Comanda c - Copy Log");
...
default: CTRL_SERIAL.println("Comanda necunoscuta ");
}
}
